...
- Rollback da implementação é por branch/worktree; no modo manual, rollback do
  produto usa instalador completo oficialmente indicado como compatível.
...
gerada, revisada e recuperável. O kit de update foi validado e preservado, mas
a geração da chave definitiva foi adiada porque a 1.0 usará instalação manual.
...
4. Se o automático for reativado, gerar par Ed25519 separado de update offline.
...
8. Preservar ativação, suporte, Discord e diagnóstico local sem licença.
...
- canal manual do Discord continua disponível sem licença;
- nenhuma consulta ou transferência automática ocorre no modo manual.

Rollback:

- reinstalar manualmente a última RC aprovada e compatível;
...
SBOM, procedência, build portátil, NSIS 3.12, instalador e self-tests
concluídos. Para o modo manual vigente, o build gera `SHA256SUMS.txt` e não
exige chave, manifesto ou assinatura de update. A cadeia Ed25519 permanece
implementada e testada para eventual reativação do modo automático.
...
8. No modo manual, gerar `SHA256SUMS.txt`; no automático futuro, assinar o
   manifesto v2 por último e offline.
...
- `SHA256SUMS.txt` publicado corresponde byte a byte ao instalador final;
- no modo automático futuro, o manifesto corresponde ao instalador final.

Rollback:

- descartar artefatos; código e produção não são afetados.

Gate: G2 encerrado para instalação manual; novo gate será necessário antes de
reativar update automático.
...
1. Publicar instalador, `SHA256SUMS.txt`, SBOM e procedência.
2. Confirmar download público manual pelo canal oficial.
3. Validar o SHA-256 do arquivo público, não apenas do arquivo local.
4. Confirmar que o cliente não consulta feed nem baixa executável.
5. Monitorar ativação, validação e erros sem telemetria indevida.
6. Preparar resposta a comprometimento da chave de lease.
...
- G2 — encerrado para instalação manual; Authenticode permanece fora do escopo
  e a chave definitiva de update só será necessária se o automático voltar.
